...
initialize() {
	...
	defineConstant("selCoef", 0.0001);
	defineConstant("balCoef", 100);
...
251000 late() {
	...
	if (size(mut) == 1){cat(seedID + "251000: Established\n");}
	else
	{
		cat(seedID + "251000: LOST – RESTARTING\n");
...
300000 late() {
	mut = sim.mutationsOfType(m2);
	if (size(mut) == 1){cat(seedID + "300000: Established\n");}
	else
	{
		cat(seedID + "300000: LOST – RESTARTING\n");
		// go back to generation of split
		sim.readFromPopulationFile(outDir+"temp_" + simID+"_" + seedID+ ".txt");
		// start a newly seeded run
		setSeed(rdunif(1, 0, asInteger(2^62) - 1));
...
350000 late() {
	...
	if (size(mut) == 1)
	{
		if (sim.mutationFrequencies(NULL, mut) > 0.001)
		{
			...
			cat(sim.mutationFrequencies(NULL, mut));
			cat(seedID + ": Not Established – RESTARTING\n");
			// go back to generation of split
			sim.readFromPopulationFile(outDir+"temp_" + simID+"_" + seedID+ ".txt");
			// start a newly seeded run
			setSeed(rdunif(1, 0, asInteger(2^62) - 1));
